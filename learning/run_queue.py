...
DEFAULT_SCRIPT = "learning/train_jax_ppo.py"
...
def _defaults(block: dict) -> tuple[str, dict, dict]:
    """(script, flags, env_overrides) from a queue's `defaults:` block."""
    return (
        block.get("script", DEFAULT_SCRIPT),
        block.get("flags", {}),
        block.get("env_overrides", {}),
    )
...
def _expand_sweep(sweep: dict, defaults: dict, start_idx: int) -> list[dict]:
    ...
    default_script, default_flags, default_overrides = _defaults(defaults)
...
def main():
    ...
    if not args.queue:
        parser.error("--queue is required.")
    ...
    # Snapshot the queue spec so tooling shows the exact spec this run executed,
    # immune to later edits of the file under learning/queues/.
    if args.queue:
        try:
