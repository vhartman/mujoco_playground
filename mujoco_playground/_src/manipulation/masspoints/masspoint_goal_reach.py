...
class MasspointGoalReach(base_goal_reach.MasspointsGoalReachEnv):
  ...
  def _get_success(
    ...
    for box_name, mocap_target, obj_body in zip(["mp_0", "mp_1"], [self._mocap_target_1, self._mocap_target_2], [self._mp_1_body, self._mp_2_body]):
    # for box_name, mocap_target, obj_body in zip(["mp_0"], [self._mocap_target_1], [self._obj_1_body]):
        ...
        masspoint_pos = data.xpos[obj_body]
        masspoint_quat = data.xquat[obj_body]
        ori_error = self._orientation_error(masspoint_quat, target_quat)
  ...
  def _get_reward(
    ...
    for i, (mocap_target, mp_body) in enumerate(zip([self._mocap_target_1, self._mocap_target_2], [self._mp_1_body, self._mp_2_body])):
    ...
    # for mocap_target, obj_body in zip([self._mocap_target_2], [self._obj_2_body]):
        ...
        box_target = reward_util.tolerance(
            jp.linalg.norm(agent_pos[:2] - target_pos[:2]),
            (0, 0.005),
            margin=0.4,
            sigmoid="reciprocal",
        )
        ...
        ori_error = self._orientation_error(data.xquat[mp_body], target_quat)
