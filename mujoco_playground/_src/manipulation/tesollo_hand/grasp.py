...
# ==============================================================================
"""Grasp task variants: different observation configurations."""
...
from mujoco_playground._src.manipulation.tesollo_hand.base_grasp import (
    GraspBase,
    default_config,
)
...
class Grasp(GraspBase):
    """Grasp: full history-based observation (today's exact obs space)."""
    ...
    def _get_obs(self, data: mjx.Data, info: dict[str, Any]) -> mjx_env.Observation:
        joint_angles = self._obs_noisy_joint_angles(data, info)
        qpos_err_hist = self._obs_qpos_error_history(joint_angles, info)
        noisy_pose = self._obs_noisy_cube_pose(data, info)
        cube_pos_err_hist = self._obs_cube_pos_error_history(noisy_pose, data, info)
        cube_ori_err_hist = self._obs_cube_ori_error_history(noisy_pose[3:], data, info)
        last_act = self._obs_last_act(info)

        state = jp.concatenate([
            joint_angles,       # NQ
            qpos_err_hist,      # NQ * history_len
            cube_pos_err_hist,  # 3 * history_len
            cube_ori_err_hist,  # 6 * history_len
            last_act,           # NQ
        ])
        privileged_state = jp.concatenate([
            state,
            self._obs_privileged_grasp(data, info),
        ])
        return {"state": state, "privileged_state": privileged_state}


def domain_randomize(model: mjx.Model, rng: jax.Array):
    mj_model = Grasp().mj_model
