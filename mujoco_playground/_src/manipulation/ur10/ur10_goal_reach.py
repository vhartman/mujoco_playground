...
OBJ_SAMPLE_MIN = (-0.4, -0.4, 0.2)
# OBJ_SAMPLE_MAX = (0.5, 0.2, 0.04)
OBJ_SAMPLE_MAX = (0.4, 0.4, 0.8)
...
class DualUR10GoalReach(ur10_base.DualUR10Base):
  ...
  def _get_rand_target_pos(
    ...
    min_pos = jp.array([-offset, -offset, -0.05]) + init_pos
    max_pos = jp.array([offset, offset, 0.05]) + init_pos
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    metrics = {
        ...
        "success_1_mp_0": jp.array(0.0, dtype=float),
        "success_2_mp_0": jp.array(0.0, dtype=float),
        "success_1_mp_1": jp.array(0.0, dtype=float),
        "success_2_mp_1": jp.array(0.0, dtype=float),
        "subsuccess": jp.array(0.0, dtype=float),
        **{k: 0.0 for k in self._config.reward_config.reward_scales.keys()},
    }
    info = {
        "rng": rng,
        "mp_0_success": jp.array(0, dtype=float),
        "mp_1_success": jp.array(0, dtype=float),
        ...
        "angle_curriculum": jp.array([20, 30, 45, 90, 135, 180], dtype=float),
        "pos_curriculum": jp.array([0.8, 0.8, 0.8, 0.8, 0.8, 0.8], dtype=float),
    }
  ...
  def _get_success(
    ...
    for i, (robot_name, mocap_target) in enumerate(zip(["mp_0", "mp_1"], [self._mocap_target_1, self._mocap_target_2])):
    # for box_name, mocap_target, obj_body in zip(["mp_0"], [self._mocap_target_1], [self._obj_1_body]):
        ...
        state.info[f"{robot_name}_success"] = jp.maximum(box_success.astype(float), state.info[f"{robot_name}_success"])

        state.metrics[f"success_1_{robot_name}"] = 1. * success_cond_1.astype(float)
        state.metrics[f"success_2_{robot_name}"] = 1. * success_cond_2.astype(float)

    success = (state.info["mp_0_success"] + state.info["mp_1_success"]) > 1.5
    # success = state.info["mp_0_success"]
    # success = state.info["mp_1_success"]
  ...
  def _reset_if_success(
    ...
    new_target_1_pos = jp.where(
        success,
        self._get_rand_target_pos(key1, max_pos, self.workspace_center),
        target_1_pos,
    )
    ...
    state.info["mp_0_success"] = state.info["mp_0_success"] * (1 - success.astype(int))
    state.info["mp_1_success"] = state.info["mp_1_success"] * (1 - success.astype(int))
  ...
  def _get_reward(
    ...
    for i, mocap_target in enumerate([self._mocap_target_1, self._mocap_target_2]):
    ...
    # for mocap_target, obj_body in zip([self._mocap_target_2], [self._obj_2_body]):
        ...
        total_ee_target += box_target * (1 - info[f"mp_{i}_success"])
  ...
  def _get_single_obs(self, data: mjx.Data, info: dict[str, Any]) -> jax.Array:
    ...
    obs = jp.concatenate([
        ...
        info["mp_0_success"].reshape(-1),
        info["mp_1_success"].reshape(-1),
    ])
    return obs
